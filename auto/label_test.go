...
func (l *ListCompleteService) GetLabels(ctx context.Context, ids ...string) map[string]*Label {
...
func (l *UserCompleteService) GetLabels(ctx context.Context, ids ...string) map[string]*Label {
	m := make(map[string]*Label)
	for _, id := range ids {
		m[id] = &Label{Id: id,
			Name: "user:" + id}
